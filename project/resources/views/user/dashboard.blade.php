...
@push('css')
<style>
.welcome-notification-style .card {
    background-color: #d1ecf1; /* Light blue background like in the screenshot */
    color: #0c5460; /* A matching dark text color for readability */
    border-color: #bee5eb;
}

.welcome-notification-style .card-body h3 {
    font-size: 1rem; /* Smaller font size for the welcome text */
}

.welcome-notification-style .btn-danger {
    background-color: #0c5460; /* Darker button to match the text */
    border-color: #0c5460;
}
</style>
@endpush
...
<div class="breadcrumb-area">
    <div>
        <h3 class="title">@lang('Welcome,') {{ auth()->user()->name }}</h3>
        <div class="mt-2">
            <a href="{{ route('user.deposit.create') }}" class="btn btn-sm btn--success mr-2">@lang('Deposit')</a>
            <a href="{{ route('user.withdraw.index') }}" class="btn btn-sm btn--danger">@lang('Withdraw')</a>
        </div>
    </div>
  <ul class="breadcrumb">
      <li>
...
@if(auth()->user()->welcome_message_seen == 0)
<div class="row mb-3 welcome-notification-style" id="welcome-notification">
    <div class="col-md-12">
        <div class="card default--card">
            <div class="card-body">
                <div class="form-group w-100 d-flex flex-wrap align-items-center justify-content-evenly justify-content-sm-between">
                    <h3 class="my-1 text-center text-sm-start">{{ __('Welcome to Genius HYIP, with a lot of Security Features.') }}</h3>
                    <div class="my-1">
                        <button class="btn btn-sm btn-danger" id="close-welcome-notification">X</button>
                    </div>
                </div>
            </div>
        </div>
...
@push('js')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const closeButton = document.getElementById('close-welcome-notification');
        if (closeButton) {
            closeButton.addEventListener('click', function() {
                fetch('{{ route('user.welcome.dismiss') }}', {
                    method: 'POST',
                    headers: {
