...
<body>
                ...
                <div class="accounts-inner__wrapper bg--section">
                    <div class="accounts-left">
                        <div class="accounts-left-content mw-100">
                            ...
                            <form class="row gy-3" action="{{ route('user.register.submit') }}" method="POST" novalidate>
                                ...
                                <div class="col-lg-6 col-lg-12 col-xl-6">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="phone_code" id="phone_code" class="form-control" readonly>
    ...
    <script>
        $(document).ready(function() {
            var countries = @json($countries->keyBy('id'));

            function updatePhoneCode() {
                var countryId = $('#country').val();
                if (countries[countryId] && countries[countryId].phone_code) {
                    $('#phone_code').val('+' + countries[countryId].phone_code);
                } else {
                    $('#phone_code').val('');
                }
            }
    ...
    $('form').on('submit', function(e) {
        ...
        var spinner = form.find('.formSpin');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success' && response.redirect) {
                    ...
                    // This part is for displaying validation errors, which is already handled
                    // by the server-side redirect for non-AJAX, but we keep it for robustness.
                    // You might need to adjust selectors to match your HTML for displaying errors.
                    $.each(response.errors, function(key, value) {
                        var errorSpan = $('<span>').addClass('text-danger').text(value[0]);
            ...
            error: function(xhr) {
                ...
                spinner.hide();
                // You can add a general error message here
            }
        });
    });
    </script>
    <!-- Remove our custom JavaScript that was causing issues -->
    <style>
        .formSpin {
