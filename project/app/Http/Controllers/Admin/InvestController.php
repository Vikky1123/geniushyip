    ...
    public function datatables(){
        ...
        return Datatables::of($datas)
                        ->editColumn('user_id', function($data) {
                            return '<div>
                                    '.ucfirst(optional($data->user)->name).'
                                    <p>@'.optional($data->user)->email.'</p>
                            </div>';
                        })
                        ->editColumn('plan_id', function($data) {
                            return  '<div>
                                        '.optional($data->plan)->title.'
                                        <p>'.showAdminAmount($data->profit_amount).' / '.optional(optional($data->plan)->schedule)->name.'</p>
                                </div>';
                        })
                        ...
                        ->editColumn('profit_time', function(Invest $data){
                            if ($data->profit_time) {
                                if (is_string($data->profit_time)) {
                                    return \Carbon\Carbon::parse($data->profit_time)->toDateString();
                                }
                                return $data->profit_time->toDateString();
                            }
                            return '--';
                        })
